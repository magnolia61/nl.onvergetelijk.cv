...
function curriculum_civicrm_custom( $op, $groupID, $entityID, &$params ) {
 ...
 //   watchdog('php', '<pre>'. print_r($groupID, TRUE) .'</pre>', NULL, WATCHDOG_DEBUG);
  ...
  if ($groupID == 103) {										   //	group ID of the Curriculum custom field set
    ...
    while ($dao1->fetch()) {									//	loop through each Curriculum record, calculating the HoeVaakLeiding in jaren
		$id 				= $dao1->id;
